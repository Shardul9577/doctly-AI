...
export default function AccountGeneral() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const rawDob = userData?.birth_date || '';
      ...
      const transformedData: FormValuesProps = {
        ...
        address: userData?.personal_address || '',
        about: userData?.about || '',
        age: userData?.age?.toString() || '',
        gender: userData?.gender || '',
        maritalStatus: userData?.marital_status || '',
        date_of_birth:
          rawDob && dayjs(rawDob, 'DD/MM/YYYY').isValid()
            ? dayjs(rawDob, 'DD/MM/YYYY').format('YYYY-MM-DD')
            : '',
        bloodGroup: userData?.blood_group || '',
        spouse_full_name: userData?.spouse_full_name || '',
      };
  ...
  const onSubmit = async (data: FormValuesProps) => {
    try {
      ...
      const formattedDate = dayjs(data.date_of_birth).format('DD/MM/YYYY');
      ...
      let res = await axiosInstance.put('/api/patients/profile', formData, {
        headers: {
      ...
      enqueueSnackbar(
        res?.data?.result?.message || 'Profile updated successfully!',
        {
          variant: 'success',
        }
      );
