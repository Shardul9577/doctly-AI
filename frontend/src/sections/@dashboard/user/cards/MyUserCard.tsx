...
export default function UserCard({ user, relationStatus }: Props) {
  ...
  const confirmDisconnect = async () => {
    try {
      const response = await axiosInstance.delete(
        `/api/doctors/patient/disconnect/${patientId}`
      );
      enqueueSnackbar(
        response.data.message || 'Patient disconnected successfully',
        {
          variant: 'success',
        }
      );
      push(PATH_DASHBOARD.docPatient.list);
      // Optionally trigger a refresh
    } catch (error) {
      console.error('Error disconnecting patient:', error);
      enqueueSnackbar(error.message || 'Failed to disconnect patient', {
        variant: 'error',
      });
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        <Box sx={{ bgcolor: 'error.main', px: 3, py: 1.5 }}>
          <Typography
            variant='subtitle1'
